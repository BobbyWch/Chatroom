...
public final class User implements java.io.Serializable {
    ...
    private User(UserInfo info) {
        ...
        permission = LocalEnum.Permission_DEFAULT;
        sentence = "个性签名";
        ...
        File userBg = null;
        for (File temp : files) {
            if (temp.getName().startsWith("User."))
                userImg = temp;
            else if (temp.getName().startsWith("UserBg."))
                userBg = temp;
        }

        try {
            bg = new ImageIcon(ImageIO.read(userBg));
            img = new ImageIcon(ImageIO.read(userImg));
    ...
    private final class UserPanel extends JPanel {
        ...
        public void resize(){
            bg=User.this.bg.getImage().getScaledInstance(getWidth(),getHeight(),Image.SCALE_DEFAULT);
            user=User.this.img.getImage().getScaledInstance(100,100,Image.SCALE_DEFAULT);
        }
    }
}